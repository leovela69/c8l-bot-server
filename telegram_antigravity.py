...
def _build_system_prompt(triage: Optional[TriageResult], user_name: str) -> str:
    ...
    base = (
        f"Eres Leon, el bot oficial de C8L Agency. "
        f"Hablas en español casual pero inteligente. Usas emojis con moderacion. "
        f"El usuario se llama {user_name}. "
        f"\n\n"
        f"TUS CAPACIDADES REALES (puedes hacer TODO esto):\n"
        f"- Crear imagenes: /imagen [descripcion]\n"
        f"- Crear musica: /musica [estilo]\n"
        f"- Crear video: /video [concepto]\n"
        f"- Transcribir audios (Whisper)\n"
        f"- Analizar fotos e imagenes (Vision)\n"
        f"- Casino: /casino o /slot\n"
        f"- Ajedrez: /chess\n"
        f"- Clima: /clima [ciudad]\n"
        f"- Crypto: /crypto [moneda]\n"
        f"- Traducir: /traducir [texto]\n"
        f"- Calculadora: /calcular\n"
        f"- Noticias: /noticias\n"
        f"- Tienda de coins: /tienda o /comprar\n"
        f"- Ver wallet: /wallet o /saldo\n"
        f"- Login diario: /daily\n"
        f"- Enviar regalos: /regalo\n"
        f"- Retirar diamantes: /retirar\n"
        f"- Codigos premium: /premium [codigo]\n"
        f"- Guia visual: /guide [tarea]\n"
        f"- Estado del sistema: /status\n"
        f"\n"
        f"REGLAS DE COMPORTAMIENTO:\n"
        f"- NUNCA digas que no puedes hacer algo que SI puedes (mira la lista arriba)\n"
        f"- Si el usuario pide algo de la lista, indicale el comando correcto\n"
        f"- Si pide 'enlace de recargas' o 'comprar coins' → dile que use /comprar\n"
        f"- Si pide su saldo → dile que use /wallet\n"
        f"- Si te manda un audio → lo transcribes automaticamente\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        "/guide [tarea] — Guia visual con screenshots\n\n"
        "💰 *Economia:*\n"
        "/wallet — Ver tu saldo (coins + diamantes)\n"
        "/daily — Login diario (+ rachas)\n"
        "/tienda — Ver packs y regalos\n"
        "/comprar — Comprar coins (PayPal)\n"
        "/regalo [gift] [user] — Regalar a alguien\n"
        "/retirar [cantidad] — Retirar diamantes a USD\n"
        "/premium [codigo] — Canjear codigo premium\n\n"
        "📊 *Sistema:*\n"
        "/status — Estado del bot\n"
        "/stats — Estadisticas detalladas\n\n"
        "🎤 *Multimedia:*\n"
        "Enviame audio → lo transcribo\n"
        "Enviame foto → la analizo\n"
        "Enviame video → lo proceso\n"
        "Enviame PDF → lo resumo\n\n"
...
async def cmd_securitylog(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Mantener proceso vivo aunque falle el bot
        while True:
            time.sleep(60)
