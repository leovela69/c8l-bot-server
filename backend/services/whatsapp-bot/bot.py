...
# Importar agentes compartidos del bot de Telegram
...
# ---------------------------------------------------------------------------
# Instancias globales (reusan los mismos agentes que Telegram)
# ---------------------------------------------------------------------------
memory = BotMemory()
...
def mark_as_read(message_id: str) -> None:
    """Marca un mensaje como leido (doble tick azul)."""
    payload = {
        "messaging_product": "whatsapp",
        "status": "read",
...
# ---------------------------------------------------------------------------
# Procesar resultado del agente y enviarlo por WhatsApp
# ---------------------------------------------------------------------------
...
def verify_webhook():
    ...
    if mode == "subscribe" and token == VERIFY_TOKEN:
        ...
        logger.warning(f"Verificacion fallida: mode={mode}, token={token}")
...
def receive_message():
    ...
    try:
        ...
        msg_id = msg.get("id")
        phone = msg.get("from")  # Numero del remitente
        ...
        if msg_type == "text":
            ...
            # (WhatsApp no tiene "typing" indicator via API, pero podemos enviar un mensaje)

            # Rastrear interaccion
            chat_id = int(phone[-10:])  # Usar ultimos 10 digitos como ID
...
if __name__ == "__main__":
    ...
    if telegram_token:
        ...
        try:
            ...
            tg_bot_path = os.path.join(BOT_DIR, "bot.py")
            spec = importlib.util.spec_from_file_location("telegram_bot_module", tg_bot_path)
            tg_bot = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(tg_bot)
