...
import { type F1Circuit, pickRandomCircuit, F1_CIRCUITS } from "../../data/f1Circuits";
...
export default function FormulaGame() {
  ...
  const [selectedCircuitId, setSelectedCircuitId] = useState<number>(0);
  ...
  const drawUrban = (
    ...
    if (circ?.id === 7) {
      // Monaco Harbor blue gradient for infield
      const grad = ctx.createLinearGradient(mx - 50, my - 50, mx + 50, my + 50);
      grad.addColorStop(0, "#011222");
      grad.addColorStop(1, "#02244a");
      buildRect(-tw); ctx.fillStyle = grad; ctx.fill();
    } else {
    ...
    if (circ?.id === 7) {
      ctx.save();
      // Yacht 1
      ctx.translate(mx - 40, my - 15);
      ctx.strokeStyle = "rgba(0, 243, 255, 0.45)"; ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(-15, 4); ctx.lineTo(15, 4); ctx.lineTo(20, -2); ctx.lineTo(-10, -2); ctx.closePath(); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0, -2); ctx.lineTo(0, -18); ctx.lineTo(8, -10); ctx.closePath(); ctx.stroke();
      ctx.restore();
      
      ctx.save();
      // Yacht 2
      ctx.translate(mx + 35, my + 15);
      ctx.strokeStyle = "rgba(212, 175, 55, 0.45)"; ctx.lineWidth = 1.5;
      ctx.beginPath();
      ctx.moveTo(-18, 4); ctx.lineTo(18, 4); ctx.lineTo(24, -2); ctx.lineTo(-12, -2); ctx.closePath(); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(0, -2); ctx.lineTo(0, -22); ctx.lineTo(10, -12); ctx.closePath(); ctx.stroke();
      ctx.restore();
    }
    ...
    if (circ?.id === 19) {
      ctx.save();
      // Puerta de Alcalá
      ctx.translate(mx - 75, my - 25);
      ctx.strokeStyle = "rgba(255, 215, 0, 0.45)"; ctx.lineWidth = 1.2;
      ctx.strokeRect(-25, -15, 50, 20);
      ctx.beginPath();
      ctx.arc(0, 5, 8, Math.PI, 0);
      ctx.arc(-14, 5, 5, Math.PI, 0);
      ctx.arc(14, 5, 5, Math.PI, 0);
      ctx.moveTo(-25, -15); ctx.lineTo(0, -25); ctx.lineTo(25, -15);
      ctx.stroke();
      ctx.restore();

      ctx.save();
      // Torres Colón
      ctx.translate(mx + 60, my + 15);
      ctx.strokeStyle = "rgba(0, 227, 253, 0.45)"; ctx.lineWidth = 1.2;
      ctx.strokeRect(-12, -22, 9, 27);
      ctx.strokeRect(1, -22, 9, 27);
      ctx.beginPath();
      ctx.moveTo(-14, -22); ctx.lineTo(12, -22); ctx.lineTo(7, -29); ctx.lineTo(-9, -29); ctx.closePath();
      ctx.fillStyle = "rgba(16, 185, 129, 0.25)"; ctx.fill(); ctx.stroke();
      ctx.restore();
    }
    ...
    buildRect(0);
    if (circ?.id === 19) {
      // Madrid Spanish flag themed track coloring
      ctx.save();
      ctx.strokeStyle = "#FFD700"; ctx.setLineDash([12, 16]); ctx.lineWidth = 2.0; ctx.stroke();
      ctx.setLineDash([]);
      buildRect(tw); ctx.strokeStyle = "#FF0000"; ctx.lineWidth = 2.5; ctx.stroke();
      buildRect(-tw); ctx.strokeStyle = "#FF0000"; ctx.lineWidth = 2.5; ctx.stroke();
      ctx.restore();
    } else {
    ...
    // Monaco Tunnel Cover Overlay
    if (circ?.id === 7) {
      ctx.save();
      const tunnelY0 = y0 + r + 10;
      const tunnelY1 = y1 - r - 10;
      const tunnelW = tw * 2.3;
      
      // Shadow / frosted glass roof
      ctx.fillStyle = "rgba(15, 15, 20, 0.92)";
      ctx.fillRect(x1 - tunnelW/2, tunnelY0, tunnelW, tunnelY1 - tunnelY0);
      
      // Warning stripes at entrance & exit
      ctx.strokeStyle = "#FFD700"; ctx.lineWidth = 3;
      ctx.beginPath(); ctx.moveTo(x1 - tunnelW/2, tunnelY0); ctx.lineTo(x1 + tunnelW/2, tunnelY0); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(x1 - tunnelW/2, tunnelY1); ctx.lineTo(x1 + tunnelW/2, tunnelY1); ctx.stroke();
      
      // Label "TUNNEL"
      ctx.fillStyle = "#FFD700"; ctx.font = "bold 8px monospace"; ctx.textAlign = "center";
      ctx.fillText("TUNNEL", x1, (tunnelY0 + tunnelY1) / 2);
      ctx.restore();
    }
  ...
  const drawCars = (ctx: CanvasRenderingContext2D, circ: F1Circuit | null | undefined) => {
    ...
    currentCars.forEach(car => {
      ...
      // Check if inside Monaco tunnel (ID 7)
      const inMonacoTunnel = circ?.id === 7 && 
        Math.abs(x - (cx + w * 0.76 / 2)) < 28 && 
        y > (cy - h * 0.70 / 2 + 38 + 10) && 
        y < (cy + h * 0.70 / 2 - 38 - 10);

      // Check if just exiting tunnel
      const justExitedTunnel = circ?.id === 7 &&
        Math.abs(x - (cx + w * 0.76 / 2)) < 28 &&
        Math.abs(y - (cy + h * 0.70 / 2 - 38 - 10)) < 12;
      ...
      if (inMonacoTunnel) {
        ctx.save();
        ctx.translate(16, 0);
        const lightGrad = ctx.createLinearGradient(0, 0, 25, 0);
        lightGrad.addColorStop(0, "rgba(255, 255, 170, 0.55)");
        lightGrad.addColorStop(1, "rgba(255, 255, 170, 0.0)");
        ctx.fillStyle = lightGrad;
        ctx.beginPath(); ctx.moveTo(0, -2); ctx.lineTo(25, -9); ctx.lineTo(25, 9); ctx.lineTo(0, 2); ctx.closePath(); ctx.fill();
        ctx.restore();
      }

      // Render exit sparks
      if (justExitedTunnel && isRacing && !car.finished) {
        for (let s = 0; s < 4; s++) {
          ctx.beginPath();
          ctx.arc(-12 - Math.random() * 8, (Math.random() - 0.5) * 6, 1.5, 0, Math.PI * 2);
  ...
  const handleStartRace = async () => {
    ...
    const circuit = selectedCircuitId === 0
      ? pickRandomCircuit()
      : (F1_CIRCUITS.find(c => c.id === selectedCircuitId) ?? pickRandomCircuit());
        ...
        <div className="flex flex-col gap-6 relative z-10">
          ...
          {!isRacing && !raceFinished && (
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                  {language === "es" ? "Elige el Circuito" : "Choose Circuit"}
                </label>
                <select
                  value={selectedCircuitId}
                  onChange={e => {
                    const cid = Number(e.target.value);
                    setSelectedCircuitId(cid);
                    const selectedCirc = cid === 0 ? null : F1_CIRCUITS.find(c => c.id === cid);
                    setTimeout(() => drawTrack(selectedCirc), 50);
                  }}
                  className="w-full p-3.5 bg-black border-2 border-zinc-800 rounded-xl text-white outline-none focus:border-[var(--color-gold)] transition text-xs cursor-pointer shadow-[3px_3px_0px_#000000] font-mono font-bold"
                >
                  <option value={0} className="bg-black text-white">
                    {language === "es" ? "🎲 Sorteo Aleatorio (19 Pistas)" : "🎲 Random Draw (19 Tracks)"}
                  </option>
                  {F1_CIRCUITS.map(c => (
                    <option key={c.id} value={c.id} className="bg-black text-white">
                      {c.nombre} ({c.ambientLabel} · {c.dificultad})
                    </option>
                  ))}
                </select>
              </div>
