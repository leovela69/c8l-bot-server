...
export default function DeviceSimulator({ children }: DeviceSimulatorProps) {
  ...
  // Auto-enable simulator if 'simulator=true' query parameter is present and format is unset
  useEffect(() => {
    if (typeof window !== "undefined" && mounted) {
      const params = new URLSearchParams(window.location.search);
      if (params.get("simulator") === "true" && deviceFormat === "unset") {
        setDeviceFormat("pc");
      }
    }
  }, [mounted, deviceFormat, setDeviceFormat]);

  if (!mounted) {
    // If client is not mounted yet, render children directly
    return <>{children}</>;
  }

  const isDark = true; // Default to dark aesthetic for cyberpunk theme
  const isFull = deviceFormat === "pc" || deviceFormat === "unset";
