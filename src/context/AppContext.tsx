...
interface AppContextType {
  ...
  bizcoinBalance: number;
  setBizcoinBalance: React.Dispatch<React.SetStateAction<number>>;
  addBizcoins: (amount: number) => void;
  deductBizcoins: (amount: number) => boolean;
  convertCoinsToBizcoins: (coinAmount: number) => boolean;
  placeCasinoBet: (amount: number) => Promise<boolean>;
  awardCasinoWin: (amount: number) => Promise<void>;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [bizcoinBalance, setBizcoinBalance] = useState<number>(0);
 
  // C8L Coins, Diamonds & Bizcoins Loader
  useEffect(() => {
    ...
    let savedBizcoins = localStorage.getItem("c8l_bizcoins");
    if (savedBizcoins === null) {
      localStorage.setItem("c8l_bizcoins", "10"); // Start with 10 free Bizcoins
      setBizcoinBalance(10);
    } else {
      setBizcoinBalance(parseInt(savedBizcoins, 10));
    }
  }, []);
  ...
  const addBizcoins = useCallback((amount: number) => {
    const currentBiz = parseInt(localStorage.getItem("c8l_bizcoins") || "0", 10);
    const newVal = currentBiz + amount;
    localStorage.setItem("c8l_bizcoins", newVal.toString());
    setBizcoinBalance(newVal);
  }, []);

  const deductBizcoins = useCallback((amount: number): boolean => {
    const currentBiz = parseInt(localStorage.getItem("c8l_bizcoins") || "0", 10);
    if (currentBiz >= amount) {
      const newVal = currentBiz - amount;
      localStorage.setItem("c8l_bizcoins", newVal.toString());
      setBizcoinBalance(newVal);
      return true;
  ...
  const convertCoinsToBizcoins = useCallback((coinAmount: number): boolean => {
    const currentCoins = parseInt(localStorage.getItem("c8l_coins") || "0", 10);
    if (currentCoins >= coinAmount) {
      const bizcoinsToAdd = Math.floor(coinAmount / 150);
      if (bizcoinsToAdd > 0) {
        const success = deductCCoins(coinAmount);
        if (success) {
          addBizcoins(bizcoinsToAdd);
          showNotification(
            language === "es"
              ? `¡Convertido exitosamente ${coinAmount} Coins a ${bizcoinsToAdd} Bizcoins (BZCN)!`
              : `Successfully converted ${coinAmount} Coins to ${bizcoinsToAdd} Bizcoins (BZCN)!`,
            "success"
          );
          return true;
        }
    ...
    showNotification(
      language === "es"
        ? "Saldo insuficiente o cantidad menor a la tasa mínima de 150 Coins."
        : "Insufficient balance or amount less than the 150 Coins minimum threshold.",
      "error"
    );
    return false;
  }, [deductCCoins, addBizcoins, language, showNotification]);

  const placeCasinoBet = useCallback(async (amount: number): Promise<boolean> => {
    const currentCoins = parseInt(localStorage.getItem("c8l_coins") || "0", 10);
    ...
    if (user) {
      try {
        ...
        if (result.success && result.coins !== undefined) {
          localStorage.setItem("c8l_coins", result.coins.toString());
          setC8lCoins(result.coins);
        } else {
          updateUserCoins(user.uid, newVal).catch(err => console.error("Sync coins failed:", err));
        }
      } catch (err) {
  ...
  const awardCasinoWin = useCallback(async (amount: number) => {
    const currentCoins = parseInt(localStorage.getItem("c8l_coins") || "0", 10);
    const newVal = currentCoins + amount;
    localStorage.setItem("c8l_coins", newVal.toString());
    setC8lCoins(newVal);
    
    // Dispatch global casino win event for Big Win and tracking
    if (typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent('c8l-casino-win', { detail: { amount } }));
    }
