...
import BoardGamesHub from "../../components/sections/BoardGamesHub";
...
type GameTab = "salaQuantum" | "roulette" | "solitaire" | "derby" | "f1" | "poker" | "bingo" | "chess" | "quantumSlot" | "sirenasSlot" | "ziriaSlot";


export default function CasinoPage() {
  const { language, user, loading, c8lCoins, setC8lCoins, addCCoins, showNotification, loginWithMockUser, bizcoinBalance } = useApp();
  ...
  useEffect(() => {
    ...
    const handleCasinoWin = (e: Event) => {
      const customEvent = e as CustomEvent<{ amount: number }>;
      const winAmount = customEvent.detail?.amount || 0;
      if (winAmount >= 250) {
    ...
    window.addEventListener('c8l-casino-win', handleCasinoWin);
    
    return () => {
      window.removeEventListener('c8l-screen-shake', handleShake);
      window.removeEventListener('c8l-casino-win', handleCasinoWin);
    };
  }, []);
  ...
  return (
    <div className={`min-h-screen text-white relative font-sans pt-32 pb-24 overflow-hidden premium-mesh-bg bg-black transition-transform duration-75 ${shakeTrigger ? "animate-camera-shake" : ""}`}>
      {shakeTrigger && (
      ...
      <div className={`container mx-auto px-6 relative z-10 transition-all duration-300 ${
        activeTab === "roulette" ? "max-w-[1400px]" : "max-w-6xl"
      }`}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-12 border-b border-white/5 pb-6">
          ...
          <div className="flex bg-black border border-white/10 rounded-full p-1 font-heading text-xs tracking-wider overflow-x-auto no-scrollbar max-w-full">
            {[
              ...
              { id: "poker", label: language === "es" ? "🎴 Póker" : "🎴 Poker" },
              { id: "bingo", label: language === "es" ? "🎟️ Bingo" : "🎟️ Bingo" },
              { id: "chess", label: language === "es" ? "♟️ Ajedrez" : "♟️ Chess" }
            ].map((tab) => (
              <button
                key={tab.id}
          ...
          {/* Coins & Bizcoin balance and refill button */}
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-black/60 border border-[var(--color-gold)]/30 text-sm font-mono text-[var(--color-gold)] box-glow-gold">
              <Coins size={16} />
              <span className="font-bold"><AnimatedNumber value={c8lCoins} /> Coins</span>
            </div>
            <div className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-purple-950/40 border border-purple-500/30 text-sm font-mono text-purple-400 box-glow-neon">
              <Cpu size={16} />
              <span className="font-bold"><AnimatedNumber value={bizcoinBalance} /> BZCN</span>
            </div>
            <button
      ...
      <AnimatePresence>
        {showBigWin && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black/85 backdrop-blur-md cursor-pointer select-none"
            onClick={() => setShowBigWin(false)}
          >
            {/* Flashing lights and background speed lines */}
            <div className="absolute inset-0 pointer-events-none anime-speed-lines opacity-65" />
            <div className="absolute inset-0 pointer-events-none bg-gradient-to-r from-purple-500/10 via-amber-500/10 to-pink-500/10 animate-pulse" />
            ...
            <motion.div
              ...
              className="relative p-8 border-4 border-[#D4AF37] bg-black rounded-3xl shadow-[0_0_50px_rgba(212,175,55,0.6)] text-center max-w-md mx-4 c8l-scanlines"
            >
              <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 text-6xl animate-bounce">
                👑👑👑
              </div>

              <span className="text-glow-neon text-xs font-mono text-[#00F3FF] uppercase tracking-widest font-black block mb-2">
                🔥 INCREÍBLE VICTORIA 🔥
              </span>
              
              <h2 className="font-heading font-black text-5xl md:text-6xl text-[#D4AF37] tracking-wider text-glow-gold uppercase my-4 animate-pulse">
                BIG WIN!
              </h2>

              <div className="py-3 px-6 bg-[#D4AF37]/10 border border-[#D4AF37]/30 rounded-xl my-4">
                <span className="text-zinc-500 text-[10px] font-mono uppercase tracking-widest block">Premio Acumulado</span>
                <strong className="text-3xl font-mono text-emerald-400 text-glow-neon block mt-1">
                  +<AnimatedNumber value={bigWinAmount} /> COINS
                </strong>
              </div>

              <p className="text-zinc-500 text-[9px] uppercase font-mono tracking-widest mt-4">
