...
export default function WatchClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    // Handle sim- prefix (simulated demo IDs: sim-2 → mock-2)
    const resolvedId = id.startsWith('sim-') ? `mock-${id.substring(4)}` : id;

    // 2. Find video details
    let currentVideo = MOCK_VIDEOS.find(v => v.id === resolvedId);
