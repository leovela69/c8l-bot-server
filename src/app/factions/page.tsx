...
export default function FactionsLandingPage() {
  const { user, c8lCoins, showNotification, language } = useApp();
  ...
  const handleJoinFaction = async (factionId: string) => {
    if (!user) {
      showNotification(language === 'es' ? 'Inicia sesión para unirte a un Bando' : 'Log in to join a Faction', 'info');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        showNotification(language === 'es' ? 'Solicitud de unión enviada. Espera aprobación del Capitán' : 'Join request sent. Wait for Captain approval', 'success');
        checkUserMembership();
      } else {
        showNotification(data.error || (language === 'es' ? 'Error al solicitar unión' : 'Error requesting join'), 'error');
      }
    } catch (e) {
      console.error('Error joining faction:', e);
      showNotification(language === 'es' ? 'Error de conexión' : 'Connection error', 'error');
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center font-mono text-gray-500">
        {language === 'es' ? 'Cargando Bandos...' : 'Loading Factions...'}
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8 border-b-4 border-[#D4AF37] pb-6">
        <div>
          <h1 className="text-3xl font-black text-[#D4AF37] flex items-center gap-2 tracking-wider">
            {language === 'es' ? '🛡️ BANDOS C8L' : '🛡️ C8L FACTIONS'}
          </h1>
          <p className="text-xs text-gray-400 mt-1 uppercase">
            {language === 'es' ? 'Únete a comunidades de batalla vocal · Gana Coins y XP Cooperativo' : 'Join vocal battle communities · Earn Coins & Co-op XP'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {!joinedFaction && (
            <button
              onClick={() => {
                if (!user) {
                  showNotification(language === 'es' ? 'Inicia sesión para fundar un Bando' : 'Log in to found a Faction', 'info');
                  return;
                }
                if (c8lCoins < 10000) {
                  showNotification(language === 'es' ? 'Se requieren 10,000 Coins para fundar un Bando' : '10,000 Coins are required to found a Faction', 'error');
                  return;
                }
            ...
            >
              <Plus size={16} /> {language === 'es' ? 'FUNDAR BANDO' : 'FOUND FACTION'}
            </button>
          )}
        </div>
      </div>
        ...
        {joinedFaction && (
          <div className="col-span-12 bg-gradient-to-r from-purple-950/20 to-black border-2 border-purple-500/30 rounded-xl p-5 flex flex-col sm:flex-row justify-between items-center gap-4 shadow-xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-black text-white">{language === 'es' ? 'MIEMBRO ACTIVO DE BANDO' : 'ACTIVE FACTION MEMBER'}</h3>
                ...
                <span className="text-[10px] text-purple-400 font-mono font-bold uppercase mt-1 block">
                  {language === 'es' ? 'RANGO' : 'RANK'}: {joinedFaction.role === 'captain' ? (language === 'es' ? '👑 Capitán' : '👑 Captain') : joinedFaction.role === 'vice-captain' ? (language === 'es' ? '⭐ Vice-Capitán' : '⭐ Vice-Captain') : joinedFaction.role === 'admin' ? '🛡️ Admin' : (language === 'es' ? 'Soldado' : 'Soldier')}
                </span>
              </div>
            </div>
            ...
            >
              {language === 'es' ? 'INGRESAR A MI BANDO' : 'ENTER MY FACTION'} <ArrowRight size={14} />
            </button>
          </div>
        )}
