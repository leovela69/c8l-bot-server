...
export default function LoungeDashboard() {
  ...
  const handleJoinRoom = async (roomId: string) => {
    if (!user) {
      showNotification(language === 'es' ? 'Inicia sesión para ingresar a una sala' : 'Log in to enter a room', 'info');
      return;
    }
    ...
    try {
      ...
      if (data.success && data.authorized) {
        ...
        showNotification(data.error || (language === 'es' ? 'Acceso denegado' : 'Access denied'), 'error');
      }
    } catch (e) {
      console.error('Error joining room:', e);
      showNotification(language === 'es' ? 'Error al unirse a la sala' : 'Error joining the room', 'error');
    }
  };
      ...
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-8 border-b-4 border-[#D4AF37] pb-6">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1 uppercase">
            {language === 'es' ? 'Salas de Karaoke Social con Sillones Premium · Inspirado por StarMaker' : 'Social Karaoke Rooms with Premium Seats · Inspired by StarMaker'}
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              if (!user) {
                showNotification(language === 'es' ? 'Por favor inicia sesión' : 'Please log in', 'info');
                return;
              }
          ...
          >
            <Plus size={16} /> {language === 'es' ? 'CREAR SALA' : 'CREATE ROOM'}
          </button>
        </div>
      </div>
        ...
        <div className="col-span-12 md:col-span-3 space-y-2">
          ...
          >
            <span className="flex items-center gap-2">
              <LayoutGrid size={16} /> {language === 'es' ? 'EXPLORAR SALAS' : 'EXPLORE ROOMS'}
            </span>
          ...
          <button
            onClick={() => {
              if (!user) {
                showNotification(language === 'es' ? 'Por favor inicia sesión para ver tus salas' : 'Please log in to see your rooms', 'info');
                return;
              }
          ...
          >
            <span className="flex items-center gap-2">
              <Users size={16} /> {language === 'es' ? 'MIS SALAS' : 'MY ROOMS'}
            </span>
          ...
          <button
            onClick={() => {
              if (!user) {
                showNotification(language === 'es' ? 'Por favor inicia sesión para ver invitaciones' : 'Please log in to see invitations', 'info');
                return;
              }
          ...
          >
            <span className="flex items-center gap-2">
              <Mail size={16} /> {language === 'es' ? 'INVITACIONES' : 'INVITATIONS'}
            </span>
