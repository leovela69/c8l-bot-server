...
export default function AdminControlCenter() {
  ...
  // UI state
  const [activeTab, setActiveTab] = useState<"users" | "commerce" | "telemetry" | "subagents" | "codes">("users");
  ...
  // Load real data from Supabase if authorized
  useEffect(() => {
    ...
    const fetchRealData = async () => {
      ...
      try {
        ...
        if (!usersErr && dbUsers) {
          const mapped = dbUsers.map((u: any) => ({
            ...
            handle: u.name || u.email.split('@')[0],
            email: u.email,
            country: 'España', // default
            device: 'Web/Bot',
            songsGenerated: u.music_generated_today || 0,
            tokens: u.coins || 0,
            banned: u.banned_until ? new Date(u.banned_until) > new Date() : false,
            logs: [
              `Plan actual del usuario: ${(u.plan || 'FREE').toUpperCase()}`,
              `Última generación de música: ${u.last_music_generated_date || 'Ninguna'}`
            ],
            plan: u.plan || 'free'
          }));
          setUsers(mapped);
      ...
      try {
        ...
        const { data: dbInvoices, error: invoicesErr } = await supabase
          .from('invoices')
          .select('*')
          .order('created_at', { ascending: false });

        if (!invoicesErr && dbInvoices) {
          const mapped = dbInvoices.map((inv: any) => ({
            txid: inv.invoice_number,
            timestamp: new Date(inv.created_at).toLocaleString(),
            uid: inv.user_id || 'Invitado/Anónimo',
            plan: inv.concept || 'Plan Premium',
            amount: Number(inv.total_amount),
            gateway: inv.stripe_payment_intent_id ? 'Stripe Success' : 'Simulado',
            renewal: 'Anual'
          }));
      ...
      // 3. Fetch Invitation Codes
      try {
        setLoadingCodes(true);
        const { data: dbCodes, error: codesErr } = await supabase
          .from('invitation_codes')
          .select('*')
          .order('created_at', { ascending: false });

        if (!codesErr && dbCodes) {
          setInvitationCodes(dbCodes);
        }
      } catch (err) {
  ...
  const handlePlanChange = async (uid: string, newPlan: string) => {
    try {
      ...
      if (error) {
        showNotification(`Error al cambiar plan: ${error.message}`, "error");
      } else {
        showNotification(`Rango del usuario actualizado a ${newPlan.toUpperCase()}`, "success");
  ...
  const handleGenerateInvitationCode = async () => {
    const code = 'C8L-PREM-' + Math.random().toString(36).substring(2, 8).toUpperCase();
    try {
      const { error } = await supabase
        .from('invitation_codes')
        .insert({
      ...
      if (error) {
        showNotification(`Error al crear código: ${error.message}`, "error");
      } else {
        showNotification(`Código creado con éxito: ${code}`, "success");
        setInvitationCodes(prev => [{ code, is_used: false, created_at: new Date().toISOString() }, ...prev]);
      }
    } catch (err: any) {
      showNotification(`Error: ${err.message}`, "error");
    }
      ...
      <div className="container mx-auto px-6 max-w-7xl relative z-10">
        ...
        <section className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4 mb-6">
          <div className="flex bg-[#0d0d10] border border-zinc-800/80 p-1 rounded-xl font-heading text-xs tracking-wider max-w-fit">
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-lg transition-all flex items-center gap-2 cursor-pointer ${
                activeTab === "codes" ? "bg-red-600 text-white font-bold" : "text-zinc-500 hover:text-white"
              }`}
            >
              <Plus size={14} />
        ...
        <section className="bg-[#121216] border border-zinc-800/40 rounded-3xl overflow-hidden shadow-2xl p-4 mb-8">
          <div className="overflow-x-auto">
                      ...
                      <tr key={user.uid} className={`hover:bg-zinc-900/30 transition-colors ${user.banned ? "bg-red-950/10 opacity-70" : ""}`}>
                        ...
                        <td className="p-4 text-right flex justify-end items-center gap-1.5">
                          <select
                            ...
                            className="bg-zinc-950 border border-zinc-850 rounded px-1.5 py-1 text-[10px] text-zinc-300 font-bold focus:outline-none focus:border-red-500 font-mono"
                          >
            ...
            {/* Tab: Codes (Invitation Codes) */}
            {activeTab === "codes" && (
              <div className="space-y-6">
                <div className="flex justify-between items-center px-4 py-2">
                  <div>
                    <h3 className="text-sm font-black text-[#D4AF37] uppercase tracking-wider font-mono">Gestión de Códigos de Invitación</h3>
                    <p className="text-[10px] text-zinc-500 font-mono mt-1">Genera códigos Premium para dar acceso manual a los usuarios.</p>
                  </div>
                  <button
                    onClick={handleGenerateInvitationCode}
                    disabled={loadingCodes}
                    className="px-4 py-2 bg-red-650 hover:bg-red-600 border border-red-500 text-white font-mono text-xs uppercase font-bold rounded-xl transition cursor-pointer flex items-center gap-1.5"
                  >
                    <span>{loadingCodes ? 'Generando...' : 'Generar Nuevo Código'}</span>
                  </button>
                </div>
                ...
                <table className="w-full text-left border-collapse text-xs">
                  <thead>
                    <tr className="border-b border-zinc-800 text-zinc-500 uppercase tracking-widest text-[9px]">
                      ...
                      <th className="p-4">Utilizado por (UUID)</th>
                      <th className="p-4">Fecha de Canje</th>
                  ...
                  <tbody className="divide-y divide-zinc-800/50 font-medium font-mono">
                    {invitationCodes.length === 0 ? (
                      <tr>
                        <td colSpan={5} className="text-center py-12 text-zinc-500 font-sans">No hay códigos de invitación generados.</td>
                      </tr>
                    ) : (
                      invitationCodes.map((c) => (
                        <tr key={c.code} className="hover:bg-zinc-900/30 transition-colors">
                          <td className="p-4 font-bold text-white tracking-widest">{c.code}</td>
                          <td className="p-4">
                            {c.is_used ? (
                              <span className="px-2 py-0.5 rounded bg-red-950/20 text-red-400 border border-red-500/10 text-[9px] font-bold uppercase font-sans">USADO</span>
                            ) : (
                              <span className="px-2 py-0.5 rounded bg-emerald-950/20 text-emerald-400 border border-emerald-500/10 text-[9px] font-bold uppercase font-sans">DISPONIBLE</span>
                            )}
                          </td>
                          <td className="p-4 text-zinc-400">{c.used_by || '—'}</td>
                          <td className="p-4 text-zinc-500">{c.used_at ? new Date(c.used_at).toLocaleString() : '—'}</td>
                          <td className="p-4 text-zinc-500">{new Date(c.created_at).toLocaleString()}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
