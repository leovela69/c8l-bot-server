...
export default function UnifiedProfile() {
  ...
  useEffect(() => {
    const fetchUnifiedAnalytics = async () => {
      try {
        // Query directa a Supabase — compatible con Firebase Hosting (output: export)
        // No usamos /api/... porque el sitio es completamente estático
        const { supabase } = await import('@/lib/supabase/client');
        
        // Obtener videos del usuario actual si está logueado, o los más vistos en general
        ...
        const { data: videosData } = await queryBuilder;
        
        if (videosData && videosData.length > 0) {
          const totalViews = videosData.reduce((sum: number, v: any) => sum + (v.views || 0), 0);
          const totalLikes = videosData.reduce((sum: number, v: any) => sum + (v.likes || 0), 0);
          const avgEngagement = totalViews > 0 ? ((totalLikes / totalViews) * 100) : 8.4;
          
          setAnalytics({
            total_reach: totalViews || 15430,
            total_engagement: parseFloat(avgEngagement.toFixed(1)) || 8.4,
            top_content: videosData.map((v: any) => ({
              title: v.title,
              views: v.views || 0,
              likes: v.likes || 0
            })),
        ...
        // Si Supabase no tiene datos, mantiene los valores ilustrativos del estado inicial
      } catch (e) {
        // Fallback silencioso — mantiene datos mock si Supabase no está configurado
        console.debug('[C8L Profile] Supabase unavailable, using mock analytics');
      }
    };
