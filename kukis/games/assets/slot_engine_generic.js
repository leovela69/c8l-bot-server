/**
 * C8L CASINO — MOTOR GENERICO DE TRAGAPERRAS (PixiJS)
 * Mismo motor probado de "El Leon Dorado", parametrizado por config
 * para poder reusarse en distintos temas (Sirenas, Ziria Coins, etc.)
 * sin duplicar la logica de RNG/paylines/animacion.
 *
 * Uso: definir window.SLOT_CONFIG antes de cargar este script (ver
 * sirenas_slot.html / ziria_coins_slot.html para ejemplos).
 */
(function(){
'use strict';

const CFG = window.SLOT_CONFIG;
if(!CFG){ console.error('SLOT_CONFIG no definido'); return; }
...
const SYMBOLS = CFG.symbols; // [{id,img,w,pay:{5:x,4:y,3:z}}, ...] primero=hero, segundo=wild, tercero=scatter

// ===== STATE =====
let credits=CFG.startCredits||150000000, chips=8250, betIdx=4;
let lastWin=0, jackpot=CFG.startJackpot||40000000, spinning=false;
...
// ===== LOAD ASSETS =====
const IMG_PATH = CFG.assetPath;
const loadEl = document.getElementById('loading');
const loadBar = document.getElementById('loadbar');

const bundle = {bg: IMG_PATH+'bg_main.png'};
SYMBOLS.forEach(s=>{ bundle[s.id]=IMG_PATH+s.img; });

PIXI.Assets.addBundle('slot', bundle);
...
PIXI.Assets.loadBundle('slot', (p)=>{
  ...
  if(loadEl) loadEl.innerHTML = '<div style="color:#f66;font-family:monospace;text-align:center;padding:20px">Error cargando el juego.<br>Recarga la pagina.</div>';
});

// Failsafe: si por lo que sea el loader nunca resuelve (bundle mal armado,
// promesa colgada), no dejar al jugador congelado para siempre en "Cargando".
setTimeout(()=>{
  if(loadEl && loadEl.style.display!=='none'){
    console.warn('Timeout cargando assets, reintentando con fallback');
    if(loadEl) loadEl.innerHTML = '<div style="color:#fc6;font-family:monospace;text-align:center;padding:20px">Esto esta tardando mas de lo normal.<br>Verifica tu conexion o recarga la pagina.</div>';
  }
}, 15000);
...
function startGame(assets){
  ...
  const COL_ACCENT = CFG.colorAccent||0xd4a017;
  const COL_ACCENT_LIGHT = CFG.colorAccentLight||0xf5d061;
  const COL_ACCENT_DIM = CFG.colorAccentDim||0x8b6914;
  const COL_TEXT = CFG.colorText||0xf5f0e8;
  ...
  SYMBOLS.forEach(sym=>{
    if(assets[sym.id]){
      texMap[sym.id]=assets[sym.id];
    } else {
      console.warn('Missing texture for symbol:', sym.id);
      const g=new PIXI.Graphics();
      g.beginFill(0x1a1200);g.lineStyle(2,COL_ACCENT);
      g.drawRoundedRect(0,0,100,100,8);g.endFill();
      const t=new PIXI.Text(sym.id,{fontSize:30,fill:COL_ACCENT_LIGHT,fontWeight:'bold'});
  ...
  // ===== HEADER =====
  const titleT=new PIXI.Text(CFG.title,{fontFamily:'Georgia,serif',fontSize:Math.min(W*0.04,22),fill:COL_ACCENT,fontWeight:'bold',letterSpacing:2});
  titleT.anchor.set(0.5,0);titleT.x=W/2;titleT.y=H*0.02;app.stage.addChild(titleT);
  const subT=new PIXI.Text(CFG.subtitle,{fontFamily:'monospace',fontSize:9,fill:COL_ACCENT_DIM,letterSpacing:3});
  ...
  const jpLbl=new PIXI.Text('JACKPOT GLOBAL',{fontFamily:'monospace',fontSize:9,fill:COL_ACCENT_DIM,letterSpacing:2});
  jpLbl.anchor.set(0.5,0);jpLbl.x=W/2;jpLbl.y=H*0.12;app.stage.addChild(jpLbl);
  const jpVal=new PIXI.Text(fmt(jackpot),{fontFamily:'Georgia,serif',fontSize:Math.min(W*0.06,30),fill:COL_ACCENT_LIGHT,fontWeight:'bold'});
  jpVal.anchor.set(0.5,0);jpVal.x=W/2;jpVal.y=H*0.12+14;app.stage.addChild(jpVal);

  // ===== REEL FRAME =====
  const frame=new PIXI.Graphics();
  frame.lineStyle(3,COL_ACCENT,0.9);
  frame.beginFill(0x030200,0.92);
  frame.drawRoundedRect(GX-12,GY-12,GW+24,GH+24,10);
  frame.endFill();
  frame.lineStyle(1.5,COL_ACCENT_LIGHT,0.3);
  frame.drawRoundedRect(GX-16,GY-16,GW+32,GH+32,13);
  app.stage.addChild(frame);

  // ===== SIDE PANELS =====
  const mBox=new PIXI.Graphics();
  mBox.lineStyle(1.5,COL_ACCENT_DIM);mBox.beginFill(0x0a0800,0.8);
  mBox.drawRoundedRect(0,0,50,60,5);mBox.endFill();
  mBox.x=GX-70;mBox.y=GY+GH/2-30;app.stage.addChild(mBox);
  const mLbl=new PIXI.Text('MULTI',{fontFamily:'monospace',fontSize:7,fill:COL_ACCENT_DIM});
  mLbl.anchor.set(0.5);mLbl.x=mBox.x+25;mLbl.y=mBox.y+12;app.stage.addChild(mLbl);
  const mVal=new PIXI.Text('X1',{fontFamily:'Georgia,serif',fontSize:16,fill:COL_ACCENT_LIGHT,fontWeight:'bold'});
  mVal.anchor.set(0.5);mVal.x=mBox.x+25;mVal.y=mBox.y+32;app.stage.addChild(mVal);
  const mSub=new PIXI.Text(CFG.multiModeLabel||'MODO\nESPECIAL',{fontFamily:'monospace',fontSize:6,fill:COL_ACCENT_DIM,align:'center'});
  mSub.anchor.set(0.5);mSub.x=mBox.x+25;mSub.y=mBox.y+50;app.stage.addChild(mSub);

  const gBox=new PIXI.Graphics();
  gBox.lineStyle(1.5,COL_ACCENT_DIM);gBox.beginFill(0x0a0800,0.8);
  gBox.drawRoundedRect(0,0,50,60,5);gBox.endFill();
  gBox.x=GX+GW+20;gBox.y=GY+GH/2-30;app.stage.addChild(gBox);
  const gLbl=new PIXI.Text('GIROS\nGRATIS',{fontFamily:'monospace',fontSize:6,fill:COL_ACCENT_DIM,align:'center'});
  gLbl.anchor.set(0.5);gLbl.x=gBox.x+25;gLbl.y=gBox.y+12;app.stage.addChild(gLbl);
  const gVal=new PIXI.Text('0',{fontFamily:'Georgia,serif',fontSize:16,fill:COL_ACCENT_LIGHT,fontWeight:'bold'});
  gVal.anchor.set(0.5);gVal.x=gBox.x+25;gVal.y=gBox.y+32;app.stage.addChild(gVal);
  const gSub=new PIXI.Text('ACTIVOS',{fontFamily:'monospace',fontSize:6,fill:COL_ACCENT_DIM});
  ...
  const infoVals={};
  infoData.forEach(o=>{
    const lbl=new PIXI.Text(o.l,{fontFamily:'monospace',fontSize:7,fill:COL_ACCENT_DIM,letterSpacing:1});
    lbl.anchor.set(0.5,0);lbl.x=o.x;lbl.y=infoY;app.stage.addChild(lbl);
    const val=new PIXI.Text(o.v,{fontFamily:'monospace',fontSize:13,fill:COL_TEXT,fontWeight:'bold'});
  ...
  spinBg.beginFill(COL_ACCENT);spinBg.drawRoundedRect(-45,-22,90,44,8);spinBg.endFill();
  spinBg.lineStyle(2,COL_ACCENT_LIGHT);spinBg.drawRoundedRect(-45,-22,90,44,8);
  ...
  function mkBtn(label,x,fn){
    const g=new PIXI.Graphics();g.lineStyle(2,COL_ACCENT_DIM);g.beginFill(0x1a1200);
    g.drawRoundedRect(-20,-20,40,40,6);g.endFill();
    g.x=x;g.y=ctrlY;g.eventMode='static';g.cursor='pointer';g.on('pointerdown',fn);
    const t=new PIXI.Text(label,{fontSize:20,fill:COL_ACCENT_LIGHT,fontWeight:'bold'});
  ...
  function doSpin(){
    ...
    setTimeout(()=>{
      spinning=false;
      let tw=0;
      const wildId=SYMBOLS[1].id, scatterId=SYMBOLS[2].id;
      for(let li=0;li<LINES;li++){
        const line=PAYLINES[li].map((row,c)=>grid[c][row]);
        let tg=null;
        for(const s of line){if(s.id!==wildId&&s.id!==scatterId){tg=s.id;break;}}
        if(!tg){if(line.every(s=>s.id===wildId))tg=wildId;else continue;}
        let cnt=0;
        for(const s of line){if(s.id===tg||s.id===wildId)cnt++;else break;}
        if(cnt>=3){const sym=SYMBOLS.find(s=>s.id===tg);if(sym&&sym.pay[cnt])tw+=sym.pay[cnt]*bet();}
      }
      let sc=0;
      for(let c=0;c<REELS;c++)for(let r=0;r<ROWS;r++)if(grid[c][r].id===scatterId)sc++;
  ...
  function winFx(amt){
    const wt=new PIXI.Text('+'+fmt(amt),{fontFamily:'Georgia,serif',fontSize:24,fill:COL_ACCENT_LIGHT,fontWeight:'bold',stroke:0x000000,strokeThickness:3});
  ...
  console.log(CFG.title+' loaded (generic engine)');
}
