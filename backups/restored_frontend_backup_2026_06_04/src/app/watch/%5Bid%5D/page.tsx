...
import { 
  ArrowLeft, Heart, MessageSquare, Share2, Compass, Cpu, 
  Send, ShieldAlert, Sparkles, Volume2, Play, AlertTriangle 
} from 'lucide-react';
...
export default function WatchPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    // Find video in MOCK list or fetch
    const currentVideo = MOCK_VIDEOS.find(v => v.id === params.id) || MOCK_VIDEOS[0];
  ...
  const handleCommentSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (isClean) {
        setComments(prev => [
          { author: "Tú (Espectador)", text: commentInput, date: "Hace un momento", isAiApproved: true },
          ...prev
        ]);
        setCommentInput("");
        setMascotState("win");
  ...
  return (
    <div className="min-h-screen bg-[#030303] text-white pt-28 pb-20 px-4 md:px-8 font-sans select-none">
      <div className="max-w-7xl mx-auto flex flex-col lg:grid lg:grid-cols-12 gap-8 relative z-10">
          ...
          <div className="border-3 border-black bg-[#0d0d0e] p-6 shadow-[4px_4px_0px_#00F3FF]">
            ...
            <div className="flex flex-wrap justify-between items-center gap-4 mt-4 pt-4 border-t border-zinc-800">
              <div className="flex items-center gap-3">
                <img src={video.user.avatar} className="w-10 h-10 rounded-full border-2 border-[#D4AF37]" alt="avatar" />
                <div className="text-left">
                  <span className="text-sm font-bold block">{video.user.name}</span>
                  <span className="text-[10px] text-zinc-500 font-mono">15.2K Subscribers</span>
                </div>
              </div>
        ...
        {/* RIGHT COLUMN: Recommended Videos & Mascot (4 columns) */}
        <aside className="lg:col-span-4 space-y-6">
          
          {/* Lion Mascot status banner widget */}
          <div className="border-3 border-black bg-[#0d0d0e] p-5 shadow-[4px_4px_0px_#D4AF37] flex flex-col items-center">
            <span className="text-[8px] font-mono text-zinc-500 uppercase tracking-widest block mb-2 font-bold">Mileoncito TV Host</span>
          ...
          <div className="space-y-4">
            <h3 className="font-heading font-black text-sm text-white uppercase tracking-wider border-b-2 border-black pb-2 text-left">
              Videos Recomendados
            </h3>
            
            <div className="space-y-3">
              {MOCK_VIDEOS.filter(v => v.id !== params.id).map(v => (
                <Link key={v.id} href={`/watch/${v.id}`}>
                  <div className="flex gap-3 bg-[#0d0d0e] hover:bg-[#151517] border border-zinc-800 hover:border-[#00F3FF] p-2 rounded-[4px] transition-all text-left group">
                    <img 
                      src={v.thumbnail_url} 
                      className="w-28 aspect-video object-cover rounded bg-black shrink-0 border border-zinc-850" 
                      alt="thumb" 
                    />
                    <div className="overflow-hidden">
                      <h4 className="text-xs font-bold text-white line-clamp-2 leading-snug group-hover:text-[#00F3FF] transition-colors">{v.title}</h4>
                      <span className="text-[10px] text-zinc-400 block mt-1">{v.user.name}</span>
                      <span className="text-[9px] text-zinc-600 block font-mono mt-0.5">{v.views.toLocaleString()} views</span>
                    </div>
                  </div>
                </Link>
              ))}
