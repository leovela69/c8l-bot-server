...
export function C8LQuantumSlot({ userCoins, setUserCoins, onWin }: SlotProps) {
  ...
  const spin = async () => {
    ...
    if (centerRow[0] === centerRow[1] && centerRow[1] === centerRow[2]) {
      ...
      if (sym === '⚛️') {
        ...
        alert(`🏆 ¡JACKPOT QUANTUM EXTRA! +${bonus} C8L Coins añadidos a tu premio!`);
      }
  ...
  return (
    <div className="border-4 border-black bg-gradient-to-b from-[#0d0d0e] to-[#050506] p-6 shadow-[8px_8px_0px_#00F3FF] rounded-xl relative overflow-hidden">
      ...
      <h2 className="font-heading text-xl font-black text-[#00F3FF] mb-4 text-center uppercase tracking-wider flex items-center justify-center gap-2">
      ...
      <div className="grid grid-cols-3 gap-2 mb-4 bg-black/60 p-2 border border-zinc-800 text-center font-mono text-xs">
        <div>
          ...
          <span className="text-[#00F3FF] font-bold">{userCoins} C8L</span>
        </div>
        <div>
        ...
        <div>
          ...
          <span className="text-[#D4AF37] font-bold">x{multiplier}</span>
        </div>
      </div>
      
      <div className="grid grid-cols-3 gap-3 bg-black p-4 border-3 border-zinc-900 rounded-lg">
            ...
            {reel.slice(1, 4).map((symbol, rowIdx) => (
              <motion.div
                ...
                className={`h-20 flex items-center justify-center text-4xl border-2 rounded-lg transition-all ${
                  rowIdx === 1 ? 'border-[#00F3FF] bg-[#00F3FF]/5 shadow-inner' : 'border-zinc-850 bg-zinc-950/40'
                }`}
      ...
      <AnimatePresence>
        {showAnimation && lastWin && (
          ...
          >
            <div className="bg-black/95 border-3 border-[#00F3FF] rounded-lg p-5 text-center shadow-[0_0_20px_rgba(0,243,255,0.4)]">
      ...
      <button
        ...
        className="w-full mt-4 py-3 bg-gradient-to-r from-[#00F3FF] to-cyan-500 text-black font-heading font-black text-sm border-2 border-black shadow-[3px_3px_0px_#000] disabled:opacity-50 disabled:cursor-not-allowed transition-all hover:scale-[1.01] cursor-pointer"
      >
