...
const EMOTIONS = [
  { id: "happy", label: "Alegre", emoji: "😊", desc: "happy" },
  { id: "sad", label: "Triste", emoji: "😢", desc: "sad" },
  { id: "angry", label: "Agresivo", emoji: "🔥", desc: "angry" },
  { id: "energetic", label: "Enérgico", emoji: "⚡", desc: "energetic" },
  { id: "melancholic", label: "Melancólico", emoji: "😔", desc: "melancholic" },
  { id: "epic", label: "Épico", emoji: "🏆", desc: "epic" },
  { id: "sensual", label: "Sensual", emoji: "💖", desc: "sensual" },
  { id: "mysterious", label: "Misterioso", emoji: "🔮", desc: "mysterious" }
];
...
export default function StudioPage() {
  const { deviceFormat, credits, deductCredits, showNotification, language, setLanguage, c8lCoins, deductCCoins } = useApp();
  ...
  const handleGenerate = () => {
    ...
    if (c8lCoins < requiredCoins) {
      showNotification(`Coins insuficientes. Necesitas ${requiredCoins} C8L Coins para generar (Saldo actual: ${c8lCoins}).`, "error");
      return;
    }

    const finalEmotionLabel = selectedEmotion ? (EMOTIONS.find(e => e.id === selectedEmotion)?.label || "Personalizada") : "Inspiradora";

    setAuthModal({
      isOpen: true,
      title: "Confirmar Generación Replicate IA",
      description: `El subagente C8L Music Cloud Engine (impulsado por la API de Replicate) sintetizará 2 mezclas completas (Mix A y Mix B) con stems de audio y voces procedurales. Se descontarán ${requiredCoins} C8L Coins de tu saldo.`,
      costText: `Costo: ${requiredCoins} C8L Coins + ${requiredCredits} Créditos IA (Saldo: ${c8lCoins} Coins, ${credits} CR)`,
      onConfirm: () => {
        setAuthModal(prev => ({ ...prev, isOpen: false }));
        
        // Deduct credits and coins
        deductCCoins(requiredCoins);
        deductCredits(requiredCredits);
        
        setIsGenerating(true);
        setGenerationProgress(0);
        showNotification("Conectando con Replicate API Engine...", "info");
        ...
        if (!customMode) {
          ...
          if (selectedEmotion) {
            detectedTheme = `${promptInput} (Emoción: ${finalEmotionLabel})`;
          }
        } else {
  ...
  return (
    <div className="bg-[#0a0a0c] text-white flex flex-col min-h-screen relative font-sans antialiased overflow-hidden select-none pt-24 md:pt-28">
        ...
        <main className="flex-grow flex flex-col overflow-y-auto px-4 py-6 md:px-8 overflow-x-hidden">
            ...
            <header className="flex justify-between items-center border-b border-zinc-800/40 pb-4 mb-6 z-10">
              <div className="flex flex-col">
                <h1 className="text-xl font-black uppercase tracking-wider text-white">
                  ...
                  {activeTab === "generator" && "Creador de Medios IA Gratis"}
                ...
                {!customMode ? (
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Emoción</label>
                      <div className="grid grid-cols-4 gap-1.5">
                        {EMOTIONS.map(emotion => (
                          <button
                            ...
                            type="button"
                            onClick={() => setSelectedEmotion(selectedEmotion === emotion.id ? "" : emotion.id)}
                            className={`flex flex-col items-center justify-center py-2 px-1 rounded-xl border text-[9px] font-black transition-all cursor-pointer ${
                              selectedEmotion === emotion.id
                                ? "bg-red-500/10 border-red-500 text-red-500 shadow-[0_0_10px_rgba(239,68,68,0.2)]"
                                : "bg-[#0d0d10] border-zinc-800 text-zinc-400 hover:border-zinc-700 hover:text-white"
                            }`}
                          >
                            <span className="text-sm mb-0.5">{emotion.emoji}</span>
                            <span className="text-[8px]">{emotion.label}</span>
                          </button>
                        ))}
                      </div>
                    </div>
          ...
          {mode === "mobile" && (
            <div className="flex-grow flex flex-col gap-4 overflow-x-hidden pt-2 pb-16">
              ...
              {/* MOBILE GENERATOR TAB */}
              {activeTab === "generator" && (
                <div className="w-full">
                  <QuantumMediaCreator mode="mobile" />
                </div>
              )}
                ...
                <div className="flex flex-col gap-4 bg-[#121216] border border-zinc-800/40 p-5 rounded-2xl shadow-xl">
                  ...
                  {!customMode ? (
                    <div className="flex flex-col gap-3">
                      ...
                      {/* Emotion tags grid */}
                      <div className="flex flex-col gap-1.5">
                        <label className="text-[8px] font-bold text-zinc-400 uppercase tracking-wider font-mono">Emoción</label>
                        <div className="grid grid-cols-4 gap-1.5">
                          {EMOTIONS.map(emotion => (
                            <button
                              key={emotion.id}
      ...
      {mode === "mobile" && (
        <nav className="fixed bottom-0 left-0 w-full bg-[#121216]/95 border-t border-zinc-800/40 flex justify-around items-center py-2.5 z-45 backdrop-blur-lg">
          ...
          <button 
            onClick={() => { setActiveTab("generator"); setMobilePlayerExpanded(false); }}
            className={`flex flex-col items-center gap-1 cursor-pointer transition-colors ${
              activeTab === "generator" ? "text-red-500" : "text-zinc-550 hover:text-white"
            }`}
          >
            <Sparkles size={18} />
            <span className="text-[8px] font-bold font-mono uppercase tracking-wider">Creador IA</span>
          </button>
          <button 
