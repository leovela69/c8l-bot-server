...
export default function AIAgentWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end font-sans">
            ...
            <div className="bg-gradient-to-r from-black via-zinc-900 to-zinc-950 px-5 py-4 border-b border-white/10 flex justify-between items-center relative z-10">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-full border border-[var(--accent-cyan,#00F3FF)]/50 bg-zinc-950 flex items-center justify-center overflow-hidden box-glow-neon">
                  <LionMascot state={mascotState} size={48} className="mt-[-2px]" />
                </div>
      ...
      {/* Floating launcher button (Cyber-Robot Avatar) */}
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="relative flex items-center justify-center w-16 h-16 bg-black/80 border border-[var(--accent-cyan,#00F3FF)] rounded-full shadow-[0_0_20px_var(--accent-cyan-shadow,rgba(0,243,255,0.4))] transition-all hover:scale-110 group cursor-pointer cyber-robot-float overflow-visible"
        style={{
      ...
      >
        ...
        {/* Animated Cyber-Robot SVG Face */}
        <svg viewBox="0 0 100 100" className="w-12 h-12 relative z-10 transition-transform duration-300 group-hover:scale-105">
          {/* Antennas / Ears */}
          <rect x="18" y="40" width="64" height="6" rx="3" fill="var(--accent-cyan,#00F3FF)" opacity="0.8" />
          <circle cx="15" cy="43" r="4" fill="var(--accent-pink,#FF007F)" className="animate-pulse" />
          <circle cx="85" cy="43" r="4" fill="var(--accent-pink,#FF007F)" className="animate-pulse" />
          
          {/* Main Head Base */}
          <rect x="25" y="25" width="50" height="42" rx="14" fill="#0c1015" stroke="var(--accent-cyan,#00F3FF)" strokeWidth="3" />
          {/* Head Visor background */}
          <rect x="32" y="32" width="36" height="18" rx="6" fill="#020406" stroke="var(--accent-cyan-border,rgba(0, 243, 255, 0.4))" strokeWidth="1.5" />
          
          {/* Glowing Eyes with Blink Animation */}
          <g>
            {isOpen ? (
              // X eyes when open/active
              <g stroke="var(--accent-pink,#FF007F)" strokeWidth="3.5" strokeLinecap="round">
                <line x1="38" y1="37" x2="44" y2="45" />
                <line x1="44" y1="37" x2="38" y2="45" />
                <line x1="56" y1="37" x2="62" y2="45" />
                <line x1="62" y1="37" x2="56" y2="45" />
              </g>
            ) : (
              // Neo cyan eyes blinking when idle
              <g fill="var(--accent-cyan,#00F3FF)" className="robot-eye">
                <circle cx="41" cy="41" r="5" filter="drop-shadow(0px 0px 4px var(--accent-cyan,#00F3FF))" />
                <circle cx="59" cy="41" r="5" filter="drop-shadow(0px 0px 4px var(--accent-cyan,#00F3FF))" />
                {/* Micro pupils */}
          ...
          {/* Mouth / LED Grid */}
          <path d="M 40 56 Q 50 60 60 56" stroke="var(--accent-cyan,#00F3FF)" strokeWidth="2.5" fill="none" strokeLinecap="round" className="animate-pulse" />
          
          {/* Cyber Neck Connector */}
          <rect x="42" y="67" width="16" height="8" fill="#15202b" stroke="var(--accent-cyan,#00F3FF)" strokeWidth="1.5" />
          <line x1="46" y1="71" x2="54" y2="71" stroke="var(--accent-pink,#FF007F)" strokeWidth="2.5" />
        </svg>
        
        {/* Glow Border Indicator */}
        <div className="absolute inset-[-2px] rounded-full border border-[var(--accent-cyan,#00F3FF)]/40 group-hover:border-[var(--accent-pink,#FF007F)]/60 transition-colors duration-300 pointer-events-none"></div>
      </button>
    </div>
  );
}
